...
import { readMessageApi } from '@/api/message';
import {getUserTicket} from "@/utils/memory";
export default  class MessageExposure {
  dotDataArr=[];
  maxNum=0;
...
//  初始化
  ...
  dot(){
    ...
    dotDataArr.forEach(item=>{
      let value = item.split('@');
      readMessageApi(value[0],value[1]).then(res=>{})
    })
  ...
  storeIntoLocalstorage(dotDataArr){
    ...
    // window.localStorages.setItem('magDotArr',dotDataArr)
      JsCookie.set('magDotArr',dotDataArr)
  }
  dotFromLocalStorage(){
    const ctmsStr  = JsCookie.get('magDotArr')

    // if(ctmsStr && getUserTicket()){
    //   // //如果有数据，就上报打点
    //   let value =ctmsStr.split(',');
    //   value.forEach(item=>{
    //     if(Number(item.split('@')['2'])===1){
    //       return;
    //     }
    //     let val = item.split('@');
    //     readMessageApi(val[0],val[1]).then(res=>{})
    //   })
    // }
  }
  beforeLeaveWebview(){
