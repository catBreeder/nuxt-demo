          ...
          <van-row gutter="6" >
            <van-col span="12" v-for="(item,index) in productList" :key="index" >
              <common-product-detail
                ...
                :item="item" :index="index" :shareSize="16" :isShowLike="false" shareText="List">
                ...
                <van-tag slot="new" size="large" color="#f00" v-if="item.listed==1"><span style="font-style: italic">LISTED</span></van-tag>
              </common-product-detail>
            </van-col>
          </van-row>
...
<script>
  ...
  import {getShopifyListApi,shopifyShareToStoreApi,shopifyStoreCountApi,getShopifyStoreListApi} from '@/api/shopify'
  ...
  export default {
    ...
    methods:{
      ...
      saveHandle(){
        this.isWaiting = true;
        shopifyShareToStoreApi(this.chooseProductID,this.storeID).then(res=>{
